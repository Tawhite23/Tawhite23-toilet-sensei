...
- `monthlyViews` … その月の視聴回数（**累計ではない**フロー値）

優先順位は「既存スナップショット（実測）> backfill > 欠損(null)」で、実測値を上書きしません。
当月の登録者数は毎回APIの実測値で更新されます。

> `monthlyViews` を累計の `viewCount` に合算してはいけません。アナリティクスの累計は
> 非公開・削除済み動画の再生数も含むため、現存する公開動画のみを数えるAPIの `viewCount` とは
> 一致しません（2026-07末時点で 138,874 対 111,260）。1本の系列に混ぜると増減が不自然になるため、
> レポートページでも別グラフ（月間視聴回数）として表示しています。
