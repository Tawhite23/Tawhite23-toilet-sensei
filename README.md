...
上の表のうち鮮度が重要なのは `live.json`（いま配信中か）と `contents.json`
（動画/配信一覧＝カレンダー）の2つです。前者は15分毎、後者は6時間毎では
追いつきません。しかも GitHub の scheduled workflow は遅延・スキップが常態なので、
`live.json` の実際の反映は**最悪35〜40分遅れ**ていました
（ブラウザ側を60秒ポーリングにしても、読む先が古いので無意味）。

日次の集計（`report.json`）や文字起こしは対象外にしています。分単位で更新する
意味が薄いか、配信後にしか作れないデータのためです。

そこでこの2つだけを Cloudflare Worker（`worker/`）に分離しています。

```
ブラウザ ──(数十秒〜数分ごと)──▶ Worker /api/live, /api/contents ──(TTLごとに1回)──▶ YouTube Data API
                                       │
...
| | 従来（cron） | Worker |
| --- | --- | --- |
| 配信状態の更新間隔 | 15分（実際は35〜40分遅延） | 60秒 |
| 動画/配信一覧の更新間隔 | 6時間 | 5分（直近分のみ） |
| 同時接続数 | 取得なし | 取得する |
| API消費 | 約375u/日（固定） | 約800〜1400u/日（アクセスが無ければ0u） |

`/api/contents` は数百本ある動画を毎回全件取り直すのではなく、6時間毎ワークフローが
生成する完全な一覧を「ベース」に、直近30件だけをYouTube APIで上書き・先頭追加する
**差分パッチ**方式です。新着動画・配信開始/終了はここに必ず含まれます。

- サイト側の入口は `src/lib/site.config.ts` の `liveApiBaseUrl`。
  **空文字の間は従来どおり `live.json` / `contents.json` に自動フォールバック**するので、
  未デプロイでも壊れません。
- 配信状態のポーリング制御は `src/lib/liveClient.ts` に一本化（ページ内で1本に集約 /
...
`live.json` / `contents.json` の既存ワークフローはフォールバック兼 `report.json` の当月スナップショット源として
